...
export interface Entry {
  ...
  funding?: string;
  /**
   * How many. Printed last on the entry's `org` line, unless it has `rows` — or
   * as a column, in a section the document sets as a table of its own the way
   * `cv/supervision.tex` sets `supervision:`.
   */
  count?: string | number;
